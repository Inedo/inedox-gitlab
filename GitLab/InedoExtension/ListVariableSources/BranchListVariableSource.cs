...
public sealed class BranchListVariableSource : DynamicListVariableType, IMissingPersistentPropertyHandler
{
    ...
    {
        var resource = SecureResource.TryCreate(SecureResourceType.GitRepository, this.ResourceName, new ResourceResolutionContext(context.ApplicationId)) as GitLabRepository;
        if (resource == null)
            throw new InvalidOperationException($"Could not find resource \"{this.ResourceName}\".");

        var client = new GitLabClient(resource, new CredentialResolutionContext(context.ApplicationId));
    ...
    {
        if (SecureResource.TryCreate(SecureResourceType.GitRepository, this.ResourceName, new ResourceResolutionContext(context.ApplicationId)) is not GitLabRepository resource 
